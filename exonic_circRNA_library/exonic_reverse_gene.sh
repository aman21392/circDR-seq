...
seq=`bedtools getfasta -fi $genome -bed coords.bed -tab|cut -f 2|awk '{ for(i=length;i!=0;i--) x=(x substr($0,i,1))}{print x;x=""}'|tr 'ATCG' 'TAGC'`
rm coords.bed
